...
class CA_NET(nn.Module):
    def __init__(self, condition_dim=100, device='cuda'):
    ...
    def encode(self, text_embedding):
        text_embedding = torch.tensor(text_embedding).to(self.device)
...
class STAGE1_G(nn.Module):   #stage 1 generator 
    ...
    def __init__(self,
                 gf_dim, condition_dim, z_dim, device = 'cuda'):
...
generator = STAGE1_G()
generator.load_state_dict(torch.load(r"c:\Users\hardi\Downloads\netG_epoch_80.pth", map_location=torch.device('cuda')))
generator.eval()
if text_prompt:
    batch_size = 1
    z_dim = 100
    noise = torch.randn(batch_size, z_dim, device=device, dtype=torch.float32)
    _, generated_image, _, _ = generator(embeddings, noise)
    generated_image = (generated_image + 1) / 2  # Normalize to [0, 1]
    generated_image = generated_image.squeeze().detach().cpu().numpy().transpose(1, 2, 0)
